BEGIN;
ALTER TABLE public.weekly_diagnostics ADD COLUMN IF NOT EXISTS policy_version integer NOT NULL DEFAULT 1;
ALTER TABLE public.weekly_diagnostics ADD COLUMN IF NOT EXISTS application_context jsonb;
ALTER TABLE public.weekly_diagnostics ADD COLUMN IF NOT EXISTS evidence jsonb;
ALTER TABLE public.weekly_diagnostics ADD COLUMN IF NOT EXISTS adjustment_preview jsonb;

-- Decisions are server-owned. Clients retain their existing owner-scoped SELECT.
REVOKE INSERT, UPDATE, DELETE ON public.weekly_diagnostics FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.weekly_diagnostics TO service_role;

CREATE OR REPLACE FUNCTION public.weekly_adjustment_context_v1(p_user_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY INVOKER SET search_path = '' AS $$
DECLARE
  p jsonb; program public.custom_programs%ROWTYPE; meal public.meal_plans%ROWTYPE;
  profile_basis jsonb; relations jsonb;
BEGIN
  SELECT to_jsonb(profiles) INTO p FROM public.profiles WHERE id=p_user_id;
  IF p IS NULL THEN RAISE EXCEPTION 'PROFILE_UNAVAILABLE' USING ERRCODE='PT409'; END IF;
  IF (SELECT count(*) FROM public.custom_programs WHERE user_id=p_user_id AND is_active) > 1
    OR (SELECT count(*) FROM public.meal_plans WHERE user_id=p_user_id AND is_active) > 1 THEN
    RAISE EXCEPTION 'AMBIGUOUS_ACTIVE_PLAN' USING ERRCODE='PT409';
  END IF;
  SELECT * INTO program FROM public.custom_programs WHERE user_id=p_user_id AND is_active;
  SELECT * INTO meal FROM public.meal_plans WHERE user_id=p_user_id AND is_active;
  SELECT jsonb_object_agg(key,value) INTO profile_basis FROM jsonb_each(p)
    WHERE key=ANY(ARRAY['calorie_goal','protein_goal','carbs_goal','fat_goal','tdee','current_weight',
      'height','birth_date','gender','objective','activity_level','dietary_type','allergies',
      'meal_preferences','onboarding_answers','fitness_level','equipment','injuries',
      'subscription_type','subscription_status','trial_ends_at','subscription_end_date']);
  SELECT coalesce(jsonb_agg(to_jsonb(c) ORDER BY c.id), '[]'::jsonb) INTO relations
    FROM public.coach_clients c WHERE client_id=p_user_id AND status='active';
  RETURN jsonb_build_object('profile',md5(profile_basis::text),
    'programId',program.id,'program',CASE WHEN program.id IS NOT NULL THEN md5(to_jsonb(program)::text) END,
    'mealPlanId',meal.id,'mealPlan',CASE WHEN meal.id IS NOT NULL THEN md5(to_jsonb(meal)::text) END,
    'coachRelation',md5(relations::text));
END $$;
REVOKE ALL ON FUNCTION public.weekly_adjustment_context_v1(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.weekly_adjustment_context_v1(uuid) TO service_role;

-- No network/AI work occurs inside this short transaction. The server prepares
-- and validates a candidate first; any conflict or write error rolls it ALL back.
...
BEGIN
  ...
  ELSIF domain='training' THEN
    IF d.ajustements ? 'calorie_goal_new' OR coalesce((d.ajustements->>'training_volume_delta_pct')::numeric,0)=0
      ...
      OR jsonb_typeof(p_candidate->'days') IS DISTINCT FROM 'array' OR jsonb_array_length(p_candidate->'days')<>7 THEN
