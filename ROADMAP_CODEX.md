...
**Statut : checklist terminée — 16 tâches sur 16 ; définition de terminé partielle en audit RC1 : production et réutilisation de `saved_meals` sont versionnées et fail-closed ; l'ADR 0007 et sa frontière read-only fixent la cible des plans, six lectures runtime dont le contrôle de génération initiale et les deux du détail client coach sont raccordées sans écriture avec schéma déployé vérifié et sémantiques legacy préservées ; le [diagnostic hebdomadaire](docs/NUTRITION_WEEKLY_DIAGNOSTIC_CHARACTERIZATION.md) n'a aucune migration de plan applicable, son agrégation `daily_food_logs` distingue zéro, inconnue et invalide et C08 résout désormais ses [objectifs Nutrition nullable](docs/NUTRITION_WEEKLY_DIAGNOSTIC_GOALS.md) sans faux zéro ; C02 Analytics est raccordé à une [agrégation journalière null-aware](docs/PROGRESSION_ANALYTICS_CALCULATIONS.md), C01 Home à son [agrégateur de mini-graphe](docs/NUTRITION_HOME_CALORIE_MINI_GRAPH.md), C09 Home propage explicitement les erreurs des deux collections de son résumé, C03 sécurise le [journal Nutrition desktop du jour](docs/NUTRITION_DESKTOP_DAY_JOURNAL.md), C04 le [graphe desktop sept jours](docs/NUTRITION_DESKTOP_WEEK_GRAPH.md), C05 le [résumé calories/macros de `NutritionTab`](docs/NUTRITION_TAB_SUMMARY.md), C06 la [lecture « Mes repas »](docs/NUTRITION_SAVED_MEALS_LIBRARY.md), C07 le [badge `macros_on_target`](docs/NUTRITION_MACROS_ON_TARGET_BADGE.md) et C10 l'[adhérence repas Analytics coach](docs/NUTRITION_COACH_ANALYTICS_MEAL_ADHERENCE.md) ; l'[audit de clôture read-only](docs/NUTRITION_READ_ONLY_CLOSURE_AUDIT.md) ne laisse aucune migration C ; la [décision des divergences historiques](docs/NUTRITION_PHASE_4_DIVERGENCE_DECISION.md) démontre que 600/500 relève d'autorités de total non versionnées et que 0/18 relève d'un alias déjà perdu, sans migration commune ou backfill honnête**
...
- [Les nouvelles et anciennes métriques concordent sur les fixtures](docs/NUTRITION_TOTAL_COMPARISON.md) — critère non satisfait : 2 divergences pleinement comparables restent visibles. La [décision Phase 4](docs/NUTRITION_PHASE_4_DIVERGENCE_DECISION.md) recommande de les conserver comme preuves : 600/500 ne permet pas de choisir entre total déclaré et recalculé, et 0/18 ne peut être reconstruit après perte de l'alias. Le snapshot v1 empêche prospectivement la seconde perte, sans requalifier l'historique.
