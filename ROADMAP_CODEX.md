...
**Statut : Phase 1 terminée — 15 tâches sur 15, rollback documenté et cinq parcours E2E locaux verts (invitation, checkout plateforme, checkout coach, push et chat)**
...
| [x] Tester checkout plateforme et coach avec identités étrangères | 2 j | Élevée | Faible | Critique | — |
| [x] Lier tous les checkouts à l'identité et aux relations serveur | 2,5 j | Élevée | Élevé | Critique | Tests checkout |
| [x] Tester les metadata et le replay du webhook Stripe | 2 j | Élevée | Moyen | Élevé | Mocks Stripe |
| [x] Restreindre les notifications aux relations autorisées | 2 j | Élevée | Moyen | Élevé | Tests push |
| [x] Contraindre les URLs de notification à des chemins internes | 1 j | Moyenne | Faible | Moyen | Contrat push |
| [x] Remplacer l'autorisation lifetime de `setup-products` par le contrat admin | 1 j | Moyenne | Moyen | Élevé | Tests admin |
| [x] Restreindre et limiter les invitations SMTP | 1,5 j | Moyenne | Moyen | Élevé | Tests SMTP |
| [x] Assainir le rendu Markdown du chat et ajouter les tests hostiles | 2 j | Élevée | Moyen | Élevé | — |
| [x] Ajouter des journaux structurés aux rejets critiques | 1 j | Moyenne | Faible | Moyen | Contrat logs |
...
Le parcours invitation Playwright est documenté dans [`docs/E2E_INVITATION_HARNESS.md`](docs/E2E_INVITATION_HARNESS.md), les checkouts dans [`docs/E2E_CHECKOUT_HARNESS.md`](docs/E2E_CHECKOUT_HARNESS.md), le push dans [`docs/E2E_PUSH_HARNESS.md`](docs/E2E_PUSH_HARNESS.md) et le chat dans [`docs/E2E_CHAT_HARNESS.md`](docs/E2E_CHAT_HARNESS.md).
...
| [x] Renforcer les tests de progression et normalisation | 2 j | Moyenne | Faible | Élevé | Modèle canonique |
| [x] [Créer les repositories programmes, séances et exercices](docs/TRAINING_REPOSITORIES.md) | 3 j | Élevée | Moyen | Élevé | Phase 2 |
| [x] Extraire session/profil de `useClientDashboard` | 1,5 j | Moyenne | Moyen | Moyen | Repositories |
...
| [x] [Extraire programme actif et navigation des jours](docs/TRAINING_ACTIVE_PROGRAM_DAY.md) | 2,5 j | Élevée | Moyen | Élevé | Modèle canonique |
...
**Statut : `met` — checklist terminée, 16 tâches sur 16 et définition de terminé techniquement satisfaite. Production et réutilisation de `saved_meals` sont versionnées et fail-closed ; l'ADR 0007 et sa frontière read-only fixent la cible des plans ; l'[audit de clôture read-only](docs/NUTRITION_READ_ONLY_CLOSURE_AUDIT.md) ne laisse aucune migration C. La [décision métier des divergences historiques](docs/NUTRITION_PHASE_4_DIVERGENCE_DECISION.md) accepte 600/500 kcal et 0/18 g comme exceptions documentées, interdit tout backfill et maintient les fixtures divergentes. L'autorité versionnée des futurs totaux de plan est un chantier distinct**
...
- [Les nouvelles et anciennes métriques sont concordantes ou couvertes par une exception métier explicite](docs/NUTRITION_TOTAL_COMPARISON.md) — critère techniquement satisfait : les deux fixtures `divergent` restent inchangées mais 600/500 kcal et 0/18 g sont acceptées comme exceptions historiques sans backfill par la [décision Phase 4](docs/NUTRITION_PHASE_4_DIVERGENCE_DECISION.md). Le snapshot v1 est l'autorité prospective des alias.
...
| Tâche | Estimation | Difficulté | Risque | Impact | Dépend de |
|---|---:|---|---|---|---|
| [x] [Écrire les E2E coach/client de caractérisation](docs/E2E_COACH_CLIENT_HARNESS.md) | 2,5 j | Élevée | Faible | Critique | Phase 2 |
...
| [x] [Ajouter pagination aux listes coach importantes](docs/COACH_LIST_PAGINATION.md) | 2 j | Moyenne | Moyen | Moyen | Repositories |
...
| [x] [Documenter le cycle de vie des abonnements](docs/BILLING_SUBSCRIPTION_LIFECYCLE.md) | 1 j | Moyenne | Faible | Élevé | Modèle final |
...
- [x] Vérifier messagerie, realtime, reconnexion et nettoyage — preuves :
  `npm run test:e2e:push`, `npm run test:e2e:chat`,
  `tests/unit/coaching-messaging-realtime.test.ts`,
...
- [x] Vérifier Progression : poids, mesures, records et analytics — calculs,
  read models, loaders dashboard, domaines détail client et agrégations
  validés. `npm run progression:authority:check` retourne `status=ok` avec
  711 consommateurs audités et 2 usages legacy intentionnels. Aucun E2E
  navigateur Progression complet identifié ; limitation documentée RC1.
- [x] Vérifier les 15 flux IA avec mocks/goldens, sans fournisseur réel —
...
- [x] Vérifier les polices locales et l’absence de Google Fonts —
  `tests/unit/local-font-hosting.test.ts` valide la configuration
...
| [x] Vérifier l'alignement migrations locales/distantes — après le constat initial `141/145 HISTORY_AND_STRUCTURE_DRIFT`, la remédiation bornée a été vérifiée par deux captures staging read-only indépendantes : `145/145 ALIGNED`, zéro manque, ajout, doublon ou divergence d'ordre et empreinte structurelle identique, sans accès Production | 1,5 j | Élevée | Moyen | Critique | Accès environnements |
...
| [ ] Ajouter les quality gates CI progressifs — `CI_STABILITY_CANDIDATE`; Gates A/B/C1/C2 versionnés, 2/2 runs complets `PASS`, p95 provisoire `16:07` et flaky rate provisoire `0 %`; attente d'un échantillon suffisant avant clôture | 2 j | Moyenne | Moyen | Élevé | Tests stables |
...
| `useClientDashboard.ts` | 644 | <250 | 3 | ✅ — 203 lignes |

## Dette restante par domaine

| Domaine | Dette initiale | Cible | Statut actuel |
|---|---|---|---|
| Sécurité préalable | Critique | Aucun P0 ouvert | 🟢 — checklist, rollback et cinq parcours E2E locaux complets |
