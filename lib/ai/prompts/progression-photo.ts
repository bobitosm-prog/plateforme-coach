...
import type { PromptImage } from './progression'

export interface ProgressPhotoProfile {
  full_name?: string | number | null; gender?: string | number | null; current_weight?: string | number | null
  height?: string | number | null; objective?: string | number | null; fitness_score?: string | number | null
  body_fat?: string | number | null; waist?: string | number | null; tdee?: string | number | null
  calorie_goal?: string | number | null; protein_goal?: string | number | null; carbs_goal?: string | number | null
  fat_goal?: string | number | null; activity_level?: string | number | null; weight_trend?: string | null
  weight_delta_30d?: string | number | null; fitness_level?: string | number | null; last_analysis?: string | null
}

export function buildProgressPhotoAssessmentInvocation(input: { profileData?: ProgressPhotoProfile | null; frontImg: PromptImage; backImg: PromptImage; sideImg: PromptImage }): AiPromptInvocation {
  const p = input.profileData || {}
      const currentWeight = parseFloat(String(p.current_weight)) || 0
...
Maximum 500 mots. Sois un vrai coach, pas un chatbot générique.`
      ...
      const content = [
        { type: 'image', source: { type: 'base64', media_type: input.frontImg.mediaType, data: input.frontImg.base64 } },
        { type: 'image', source: { type: 'base64', media_type: input.backImg.mediaType, data: input.backImg.base64 } },
        { type: 'image', source: { type: 'base64', media_type: input.sideImg.mediaType, data: input.sideImg.base64 } },
        { type: 'text', text: assessmentPrompt },
      ]

  return immutableInvocation({ model: 'claude-opus-4-8', max_tokens: 2048, system: assessmentSystem, messages: [{ role: 'user', content }] })
}

export function buildProgressPhotoInvocation(input: { profileData?: ProgressPhotoProfile | null; mainImage: PromptImage; previousImage?: PromptImage | null }): AiPromptInvocation {
  const p = input.profileData || {}
    // ── System prompt ──
...
9. Réponds UNIQUEMENT en français`
    ...
    const height = parseFloat(String(p.height)) || 0
    const bmi = height > 0 ? currentWeight / ((height / 100) ** 2) : 0
    const tdee = parseFloat(String(p.tdee)) || 0
    const calorieGoal = parseFloat(String(p.calorie_goal)) || 0
    const surplusDeficit = tdee > 0 && calorieGoal > 0 ? calorieGoal - tdee : null
    const proteinGoal = parseFloat(String(p.protein_goal)) || 0
...
Maximum 400 mots. Sois un vrai coach, pas un chatbot générique.`
  ...
  const text = input.previousImage ? userPrompt.replace('Analyse cette photo', 'Compare ces deux photos de progression (avant → après)') : userPrompt
  const content = input.previousImage ? [
    { type: 'image', source: { type: 'base64', media_type: input.previousImage.mediaType, data: input.previousImage.base64 } },
    { type: 'image', source: { type: 'base64', media_type: input.mainImage.mediaType, data: input.mainImage.base64 } },
    { type: 'text', text },
  ] : [
    { type: 'image', source: { type: 'base64', media_type: input.mainImage.mediaType, data: input.mainImage.base64 } },
    { type: 'text', text },
  ]
  return immutableInvocation({ model: 'claude-opus-4-8', max_tokens: 1024, system: systemPrompt, messages: [{ role: 'user', content }] })
}
