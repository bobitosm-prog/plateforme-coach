...
const ALLERGEN_TERMS: Record<string, string[]> = {
  gluten: ['blé', 'ble', 'orge', 'seigle', 'épeautre', 'epeautre'], lactose: ['lait', 'yaourt', 'fromage', 'whey'],
...
export function validateAthenaNutritionDay(value: unknown, targets: NutritionTargets) {
  ...
  return { ...parsed.data, total_kcal: Math.round(totals.kcal), total_protein: Math.round(totals.protein), total_carbs: Math.round(totals.carbs), total_fat: Math.round(totals.fat) }
}
