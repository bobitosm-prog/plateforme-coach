        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {Array.from({ length: TOTAL_STEPS }, (_, i) => (
            <div key={i} style={{ flex: 1, height: 3, borderRadius: 0, background: BORDER, overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 0, background: GOLD, width: i < step ? '100%' : '0%', transition: 'width 400ms ease' }} />
            </div>
          ))}
        </div>
                  ...
                  {MAX_CLIENT_OPTIONS.map(opt => {
                    ...
                    return (
                      <button key={opt} onClick={() => setMaxClients(opt)}
                        style={{ padding: '10px 16px', borderRadius: 0, border: `1.5px solid ${sel ? GOLD : BORDER}`, background: sel ? GOLD_DIM : BG_CARD, cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, fontFamily: FONT_ALT, color: sel ? GOLD : TEXT_PRIMARY, transition: 'all 200ms' }}>
                        {opt}
                      </button>
                    )
                  })}
                  ...
                  {DAYS.map(day => {
                    ...
                    return (
                      <button key={day} onClick={() => toggleDay(day)}
                        style={{ padding: '10px 14px', borderRadius: 0, border: `1.5px solid ${sel ? GOLD : BORDER}`, background: sel ? GOLD_DIM : BG_CARD, cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600, fontFamily: FONT_ALT, color: sel ? GOLD : TEXT_PRIMARY, transition: 'all 200ms', minWidth: 44, textAlign: 'center' }}>
                        {day}
                      </button>
                    )
                  })}
                  ...
                  {FOLLOW_UP_MODES.map(mode => {
                    ...
                    return (
                      <button key={mode} onClick={() => toggleFollowUp(mode)}
                        ...
                        <div style={{ width: 20, height: 20, borderRadius: 0, border: `2px solid ${sel ? GOLD : BORDER}`, background: sel ? GOLD : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 200ms' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/join?coach=${session.user.id}`); setInviteLinkCopied2(true); setTimeout(() => setInviteLinkCopied2(false), 2000) }}
                      style={{ padding: '12px 18px', background: inviteLinkCopied2 ? GREEN : GOLD, border: 'none', borderRadius: 0, color: BG_BASE, fontFamily: FONT_ALT, fontWeight: 800, fontSize: '0.85rem', cursor: 'pointer', transition: 'background 200ms', whiteSpace: 'nowrap', clipPath: 'polygon(0 0,100% 0,96% 100%,0 100%)' }}>
            ...
            <motion.div key="s3" custom={dir} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={transition}
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} style={{ width: '100%', maxWidth: 320, display: 'flex', flexDirection: 'column', gap: 12, marginTop: 8 }}>
                <motion.button whileTap={{ scale: 0.97 }} onClick={handleStripeConnect} disabled={stripeLoading || stripeConnected}
                  style={{ width: '100%', padding: '16px', background: stripeConnected ? GREEN : GOLD, border: 'none', borderRadius: 0, color: stripeConnected ? '#fff' : BG_BASE, fontSize: '1rem', fontWeight: 800, fontFamily: FONT_ALT, cursor: stripeLoading ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, letterSpacing: '2px', clipPath: 'polygon(0 0,100% 0,96% 100%,0 100%)' }}>
                ...
                <button onClick={goNext}
                  style={{ width: '100%', padding: '14px', background: 'transparent', border: `1.5px solid ${GOLD_RULE}`, borderRadius: 0, color: TEXT_MUTED, fontSize: '0.92rem', fontFamily: FONT_ALT, fontWeight: 600, cursor: 'pointer', transition: 'all 200ms' }}>
            ...
            <motion.div key="s4" custom={dir} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={transition}
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                ...
                <button onClick={copyInviteLink}
                  ...
                  <div style={{ width: 40, height: 40, borderRadius: 0, background: GOLD_DIM, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} style={{ width: '100%' }}>
                <motion.button whileTap={{ scale: 0.97 }} onClick={finish} disabled={saving}
                  style={{ width: '100%', padding: '18px', background: saving ? BORDER : GOLD, border: 'none', borderRadius: 0, color: saving ? TEXT_MUTED : BG_BASE, fontSize: '1.1rem', fontWeight: 800, fontFamily: FONT_ALT, cursor: saving ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, letterSpacing: '2px', clipPath: 'polygon(0 0,100% 0,96% 100%,0 100%)' }}>
...
const inputStyle: React.CSSProperties = { width: '100%', padding: '14px 16px', background: BG_BASE, border: `1.5px solid ${BORDER}`, borderRadius: 0, color: TEXT_PRIMARY, fontSize: '1rem', fontFamily: FONT_BODY, fontWeight: 300, outline: 'none', transition: 'border-color 200ms, box-shadow 200ms', boxSizing: 'border-box' }
