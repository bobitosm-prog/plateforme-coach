...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: t('privacy.title') + ' | MoovX' }
}

export default async function PrivacyPage({ params }: Props) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'legal' })
  const filePath = path.join(process.cwd(), 'content', 'legal', `privacy-${locale}.md`)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0D0B08', color: '#F0EDE8', padding: '80px 24px' }}>
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        <p style={{ color: '#8A8580', fontSize: 12, marginBottom: 8 }}>{t('privacy.version')}</p>
        <div
