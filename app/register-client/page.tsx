...
export default function RegisterClientPage() {
  ...
  const redirectUrl = typeof window !== 'undefined' ? `${window.location.origin}/onboarding` : '/onboarding'
  ...
  async function handleEmailSignUp() {
    ...
    const { error: signUpError } = await supabase.auth.signUp({ email: email.trim(), password, options: { emailRedirectTo: redirectUrl } })
    setSubmitting(false)
    if (signUpError) {
