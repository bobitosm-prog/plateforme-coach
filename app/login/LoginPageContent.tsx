...
export default function LoginPageContent() {
  ...
  async function handleEmailLogin() {
    ...
    if (signInError) {
      ...
      setSubmitting(false)
      if (signInError.message.includes('Invalid login')) setError(t('errors.invalidCredentials'))
      else if (signInError.message.includes('Email not confirmed')) setError(t('errors.emailNotConfirmed'))
            ...
            <div style={{ animation: 'fadeUp 0.7s cubic-bezier(0.16,1,0.3,1)' }}>
              ...
              {resetSent ? (
                <div style={{ textAlign: 'center', padding: '20px 0' }}>
                  ...
                  <p style={{ ...bodyStyle, fontSize: '0.9rem', color: TEXT_PRIMARY, lineHeight: 1.6 }}>{t('forgot.emailSent')} <strong style={{ color: GOLD }}>{resetEmail}</strong></p>
                  <p style={{ ...mutedStyle, fontSize: '0.78rem', marginTop: 8 }}>{t('forgot.checkInbox')}</p>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <button className="gold-btn" onClick={handleResetPassword}>{t('forgot.sendLink')}</button>
                </div>
              )}
            ...
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, margin: '8px 0', animation: 'fadeUp 0.7s 0.2s cubic-bezier(0.16,1,0.3,1) both' }}>
                  ...
                  <span style={{ ...subtitleStyle, fontSize: '0.72rem', fontWeight: 500, letterSpacing: 1, color: TEXT_DIM }}>{t('or')}</span>
                  <div style={{ flex: 1, height: 1, background: BORDER }} />
                </div>
...
function LeftPanel({ t }: { t: ReturnType<typeof useTranslations<'auth.login'>> }) {
  return (
    <div className="auth-left" style={{ width: '50%', flexShrink: 0, background: BG_BASE, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', animation: 'panelIn 0.6s cubic-bezier(0.16,1,0.3,1)' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', padding: '0 40px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 28 }}>
          ...
          <span style={{ ...labelStyle, fontSize: '0.7rem', letterSpacing: 4, fontWeight: 400 }}>{t('panel.swissMade')}</span>
        </div>
      ...
      <div style={{ position: 'absolute', bottom: 32, left: 0, right: 0, textAlign: 'center' }}>
        <span style={{ ...mutedStyle, fontSize: '0.68rem', fontWeight: 300, letterSpacing: 1 }}>{t('panel.copyright')}</span>
      </div>
    </div>
  )
}
