...
function nutritionFromFoodLogs(rows: readonly Record<string, unknown>[]): HomeSupplementalData['loggedNutrition'] {
  return rows.reduce<HomeSupplementalData['loggedNutrition']>((total, row) => ({
...
export function resolveHomeNutritionRead({
  ...
  values: HomeSupplementalData['loggedNutrition']
  hasPersonalMealPlan: boolean
...
export default function useHomeDashboardModel({
  ...
  return useMemo(() => {
    const currentSupplemental = requestKey === null
      ...
      : supplemental
    const logged = currentSupplemental.data.loggedNutrition
    const hasLoggedNutrition = Object.values(logged).some(value => value > 0)
    ...
    return buildHomeViewModel({
      ...
      nutrition: {
        ...
        caloriesConsumed: baseConsumed == null
          ? (hasLoggedNutrition ? logged.calories : null)
          : baseConsumed + logged.calories,
        macrosConsumed: {
          protein: baseMacros.protein == null && !hasLoggedNutrition ? null : (baseMacros.protein ?? 0) + logged.protein,
          carbs: baseMacros.carbs == null && !hasLoggedNutrition ? null : (baseMacros.carbs ?? 0) + logged.carbs,
          fat: baseMacros.fat == null && !hasLoggedNutrition ? null : (baseMacros.fat ?? 0) + logged.fat,
        },
