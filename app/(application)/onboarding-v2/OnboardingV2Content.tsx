...
import { GOALS, GOAL_TO_OBJECTIVE, ACTIVITY_OPTS, DIETARY_PATTERN_OPTS, NUTRITION_OPTS, EXPERIENCE_OPTS } from '@/lib/onboarding-options'
...
export default function OnboardingV2Content() {
  ...
  const [sessionsPerWeek,setSessionsPerWeek]=useState(3); const [nutrition,setNutrition]=useState<number|null>(null); const [dietaryPattern,setDietaryPattern]=useState<number|null>(null)
  ...
  useEffect(()=>{ let mounted=true; void (async()=>{
    ...
    const primaryGoalId=typeof a.primary_goal_id==='string'?a.primary_goal_id:null;const storedGoalIndex=GOALS.findIndex(o=>o.id===primaryGoalId);const gi=storedGoalIndex>=0?storedGoalIndex:primaryGoalId?null:GOALS.findIndex(o=>GOAL_TO_OBJECTIVE[o.id]===p.objective);setGoal(gi===null||gi<0?null:gi)
    const ai=ACTIVITY_OPTS.findIndex(o=>o.dbLabel===p.activity_level);setActivityLevel(ai<0?null:ai)
    const storedHabitId=typeof a.nutrition_habit_id==='string'?a.nutrition_habit_id:null;const storedHabitIndex=NUTRITION_OPTS.findIndex(o=>o.id===storedHabitId);const legacyHabitIndex=NUTRITION_OPTS.findIndex(o=>o.dbLabel===p.dietary_type);setNutrition(storedHabitIndex>=0?storedHabitIndex:legacyHabitIndex>=0?legacyHabitIndex:null)
    const dietaryPatternIndex=DIETARY_PATTERN_OPTS.findIndex(o=>o.dbLabel===p.dietary_type);setDietaryPattern(dietaryPatternIndex<0?null:dietaryPatternIndex)
  ...
  const macros=useMemo(()=>{if(!weight||!height||!birthDate||!gender||activityLevel===null||goal===null)return null;const age=Math.floor((Date.now()-new Date(birthDate).getTime())/31557600000),w=Number(weight),h=Number(height);if(!w||!h||age<=0)return null;const targets=calculateAutomaticCalorieMacroTargets({gender,age,heightCm:h,weightKg:w,activityLevel:ACTIVITY_OPTS[activityLevel].dbLabel,objective:GOAL_TO_OBJECTIVE[GOALS[goal].id],dietaryType:dietaryPattern===null?null:DIETARY_PATTERN_OPTS[dietaryPattern].dbLabel});return{tdee:targets.tdee,calorieGoal:targets.targetCalories,protein:targets.proteinGrams,fat:targets.fatGrams,carbs:targets.carbsGrams}},[weight,height,birthDate,gender,activityLevel,goal,dietaryPattern])
  ...
  async function save(){if(!flow||!userId)return false;const total=flow==='solo'?SOLO_TOTAL_STEPS:COACH_MANAGED_TOTAL_STEPS;const next=editingFromSummary?total:Math.min(step+1,total)
    ...
    if(step===1&&goal!==null){answersRef.current={...answersRef.current,athena_contract_version:1,primary_goal_id:GOALS[goal].id};return persist({objective:GOAL_TO_OBJECTIVE[GOALS[goal].id]},next)}
    ...
    if(step===4&&nutrition!==null&&dietaryPattern!==null&&restrictions!=='unanswered'){answersRef.current={...answersRef.current,nutrition_habit_id:NUTRITION_OPTS[nutrition].id};return persist({dietary_type:DIETARY_PATTERN_OPTS[dietaryPattern].dbLabel,meal_preferences:{...mealPrefs,disliked_foods:dislikedFoods,dietary_restrictions:restrictions==='some'?restrictionDetails.trim():''}},next)}
    if(step===5&&macros){if(!await initializeTrial(supabase))return false;const ok=await persist({tdee:macros.tdee,calorie_goal:macros.calorieGoal,protein_goal:macros.protein,carbs_goal:macros.carbs,fat_goal:macros.fat,onboarding_completed:true,onboarding_completed_at:new Date().toISOString(),next_diagnostic_at:new Date(Date.now()+604800000).toISOString(),needs_initial_generation:true});if(ok){invalidateProfileCache();cache.remove(`dashboard_${userId}`)}return ok}return false}
  ...
  async function upload(file:File,avatar=false){if(!userId)return;setUploadingPhoto(true);try{const path=await uploadPhoto(file,avatar?'avatars':'progress-photos');if(avatar){const publicUrl=supabase.storage.from('avatars').getPublicUrl(path).data.publicUrl;if(!await persist({avatar_url:publicUrl}))throw new Error();setAvatarUrl(publicUrl)}else{const {error:i}=await supabase.from('progress_photos').insert({user_id:userId,date:new Date().toISOString().slice(0,10),photo_url:path,view_type:'front'});if(i)throw i;const {data}=await supabase.storage.from('progress-photos').createSignedUrl(path,3600);setPhotoBodyUrl(data?.signedUrl||'')}}catch{setError(t('redesign.errors.photo'))}finally{setUploadingPhoto(false)}}
  ...
  const total=flow==='solo'?SOLO_TOTAL_STEPS:COACH_MANAGED_TOTAL_STEPS, valid=flow==='coachManaged'?(step!==1||firstName.trim().length>=2):[goal!==null,firstName.trim().length>=2&&!!birthDate&&!!gender&&Number(weight)>0&&Number(height)>0&&Number(goalWeight)>0,activityLevel!==null&&experience!==null&&locationIndex!==null,nutrition!==null&&dietaryPattern!==null&&restrictions!=='unanswered'&&(restrictions==='none'||restrictionDetails.trim().length>0),!!macros][step-1]
