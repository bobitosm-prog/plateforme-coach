...
function CoachAppContent() {
  ...
  const [nutritionQuickAction, setNutritionQuickAction] = React.useState<'photo' | null>(null)
  ...
  return (
    ...
    <div className="app-shell" style={{ display: 'flex', width: '100%', background: BG_BASE, color: TEXT_PRIMARY, fontFamily: FONT_BODY }}>
      ...
      {/* ── TRIAL BANNER ── */}
      {h.isInTrial && (
        <div style={{ flexShrink: 0, padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, background: h.trialDaysLeft <= 3 ? 'rgba(239,68,68,0.05)' : GOLD_DIM }}>
...
{h.activeTab === 'training_program' && <TrainingProgramSection activeProgram={h.activeTrainingProgram} capabilities={h.capabilities} profileObjective={h.profile?.objective} profile={h.profile} supabase={h.supabase} session={h.session} onRefresh={h.fetchAll} onBack={() => navigateTo('training')} configureOpen={navigation.section === 'training-program' && navigation.mode === 'configure'} onConfigureChange={(open) => navigateApp({ tab: 'account', section: 'training-program', mode: open ? 'configure' : undefined })} />}
            </motion.div>
          </AnimatePresence>
        </main>
      )}
