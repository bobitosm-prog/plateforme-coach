...
export default function TodayHero({ training, onStartSession, onOpenSession, onOpenProgram, onStartFreeSession }: TodayHeroProps) {
  ...
  if (viewState === 'error') return <section className={`${styles.hero} ${styles.error}`} role="status">
    ...
    {onOpenProgram && <div className={styles.actions}><button className={`${styles.button} ${styles.secondary}`} onClick={onOpenProgram}>{t('openProgram')}</button></div>}
  </section>
  ...
  if (viewState === 'completed' && session) return <section className={`${styles.hero} ${styles.success}`}>
    ...
    {onOpenSession && <div className={styles.actions}><button className={`${styles.button} ${styles.secondary}`} onClick={() => onOpenSession(session)}>{t('viewSession')} <ChevronRight size={16} aria-hidden="true" /></button></div>}
  </section>
  ...
  if (viewState === 'scheduled' && session) return <section className={styles.hero}>
    ...
    {onStartSession && <div className={styles.actions}><button className={`${styles.button} ${styles.primary}`} onClick={() => onStartSession(session)}><Play size={16} fill="currentColor" aria-hidden="true" /> {t('start')}</button></div>}
  </section>
  ...
  return <section className={styles.hero}>
    ...
    <div className={styles.actions}>
      {onStartFreeSession && <button className={`${styles.button} ${styles.primary}`} onClick={onStartFreeSession}><Dumbbell size={16} aria-hidden="true" /> {t('freeSession')}</button>}
      {onOpenProgram && <button className={`${styles.button} ${styles.secondary}`} onClick={onOpenProgram}>{t('openProgram')}</button>}
    </div>
  </section>
}
