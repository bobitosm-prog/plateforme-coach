    ...
    <AnimatePresence>
      {achievement && (
        <motion.div initial={{ opacity: 0, y: -60, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -40, scale: 0.9 }} transition={{ type: 'spring', damping: 20, stiffness: 300 }}
          ...
          <div>
            <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: 3, color: colors.goldContainer, textTransform: 'uppercase' }}>Achievement debloque !</div>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 20, letterSpacing: 1, color: '#F5EDD8' }}>{achievement.name}</div>
            <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: colors.goldContainer, fontWeight: 600 }}>+{achievement.xp_reward} XP</div>
          </div>
        </motion.div>
      )}
    </AnimatePresence>
