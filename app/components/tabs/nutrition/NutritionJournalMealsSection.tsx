...
interface ConsumedTotals { kcal: number | null; protein: number | null; carbs: number | null; fat: number | null }
interface Labels {
  noFood: string; recommended: (values: MealTotals) => string; consumed: (values: ConsumedTotals) => string
...
function sum(logs: NutritionJournalLog[]): ConsumedTotals {
  return {
    kcal: sumNutritionTabMetric(logs, 'calories'),
    protein: sumNutritionTabMetric(logs, 'protein'),
    carbs: sumNutritionTabMetric(logs, 'carbs'),
    fat: sumNutritionTabMetric(logs, 'fat'),
  }
}
