...
export interface ReusableMeal { id: string; name?: string; foods?: OverlayFood[]; use_count?: number; total_calories?: number }

interface Props {
  editingMeal: EditableMeal | null; editQuery: string; editResults: FoodSearchResult[]; editSaving: boolean; editSaved: boolean
  savedMealError: string | null; saveMealSaving: boolean
...
export function NutritionTabOverlays(props: Props) {
  return <>
    ...
    {props.savedOpen && <RailOverlay><><div onClick={props.onCloseSaved} style={backdrop} /><div style={{ ...popup, maxHeight: '75vh', overflow: 'hidden' }}><ModalHeader title={props.labels.savedTitle} onClose={props.onCloseSaved} /><div style={{ padding: 20 }}>{props.savedMeals.length === 0 ? <div style={bodyStyle}>{props.labels.savedEmpty}</div> : props.savedMeals.map(meal => <button key={meal.id} onClick={() => props.onApplySaved(meal)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between' }}><div>{meal.name}<div style={mutedStyle}>{props.labels.savedCount(meal.foods?.length ?? 0)}{(meal.use_count ?? 0) > 0 && ` · ${props.labels.usedCount(meal.use_count ?? 0)}`}</div></div><div style={statSmallStyle}>{Math.round(meal.total_calories ?? 0)}</div></button>)}</div></div></></RailOverlay>}
  </>
}
