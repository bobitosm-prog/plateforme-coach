...
interface Props {
  ...
  savedMealError: string | null; saveMealSaving: boolean; savedMealReusing: boolean
...
export function NutritionTabOverlays(props: Props) {
  return <>
    ...
    {props.savedOpen && <RailOverlay><><div onClick={props.onCloseSaved} style={backdrop} /><div style={{ ...popup, maxHeight: '75vh', overflow: 'hidden' }}><ModalHeader title={props.labels.savedTitle} onClose={props.onCloseSaved} /><div style={{ padding: 20 }}><SavedMealWriteAlert message={props.savedMealError} />{props.savedMeals.length === 0 ? <div style={bodyStyle}>{props.labels.savedEmpty}</div> : props.savedMeals.map(meal => <button key={meal.id} disabled={props.savedMealReusing} onClick={() => props.onApplySaved(meal)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between' }}><div>{meal.name}<div style={mutedStyle}>{props.labels.savedCount(meal.foods?.length ?? 0)}</div></div><div style={statSmallStyle}>{Math.round(meal.total_calories ?? 0)}</div></button>)}</div></div></></RailOverlay>}
  </>
}
