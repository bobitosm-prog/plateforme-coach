...
import {
  ...
  type NutritionTabValues,
} from '@/lib/nutrition/nutrition-tab-summary'

export type NutritionMacroTotals = NutritionTabValues
...
export function NutritionSummarySection(props: NutritionSummarySectionProps) {
  ...
  const caloriePct = nutritionTabPercentage(consumed.kcal, targets.kcal)
  ...
  return <>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 24 }}>
      <div style={{ position: 'relative' }}>
        <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
          ...
          {caloriePct !== null && <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke="url(#nutRingGrad)" strokeWidth={stroke} strokeDasharray={circumference} strokeDashoffset={circumference - caloriePct / 100 * circumference} style={{ transition: 'stroke-dashoffset 0.8s ease', filter: `drop-shadow(0 0 8px ${colors.goldRule})` }} />}
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}><span style={{ ...statStyle, fontSize: 40, color: colors.gold, lineHeight: 1 }}>{presentNutritionTabMetric(consumed.kcal)}</span><span style={{ ...mutedStyle, fontSize: 11 }}>{targets.kcal === null ? '—' : `/ ${targets.kcal} kcal`}</span><span style={{ ...mutedStyle, fontSize: 10, marginTop: 2 }}>{remainingLabel}</span></div>
      </div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
      {[
        ...
        const percentage = nutritionTabPercentage(current, target)
        return <div key={label} style={{ textAlign: 'center' }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, marginBottom: 4 }}><Icon size={12} color={color} /><span style={{ ...subtitleStyle, fontSize: 10, letterSpacing: '0.1em' }}>{label}</span></div><div style={{ ...statSmallStyle, color }}>{presentNutritionTabMetric(current, { rounded: true })}<span style={{ fontSize: 12, color: colors.textMuted }}>{target === null ? '/—' : `/${target}g`}</span></div><div style={{ height: 4, background: 'rgba(255,255,255,0.04)', borderRadius: 12, overflow: 'hidden', marginTop: 4 }}>{percentage !== null && <div style={{ height: '100%', background: color, width: `${percentage}%`, borderRadius: 12 }} />}</div></div>
      })}
    </div>
  </>
}
