        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px' }}>
          ...
          {foods.map((f, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: i < foods.length - 1 ? `1px solid ${GOLD_DIM}` : 'none' }}>
              <div>
                <div style={{ fontFamily: FONT_BODY, fontSize: 14, color: TEXT_PRIMARY, fontWeight: 500 }}>{f.name}</div>
                <div style={{ fontFamily: FONT_BODY, fontSize: 12, color: TEXT_MUTED }}>{f.qty}g</div>
              </div>
