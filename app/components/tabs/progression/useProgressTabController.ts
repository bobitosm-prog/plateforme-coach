...
export function useProgressTabController(props: ProgressTabPublicProps, t: ProgressTranslate) {
  ...
  function closeCompare() { setShowCompare(false); setAlignment(null); setAlignError(null) }
  async function alignPhotos() { const before = props.progressPhotos[compareIdx[0]], after = props.progressPhotos[compareIdx[1]]; if (!before || !after) return; setIsAligning(true); setAlignError(null); try { const result = await computeAlignment(signedUrls[before.id] ?? '', signedUrls[after.id] ?? ''); if (!result) { setAlignError(t('tab.analysisError')); return } setAlignment(result); await props.supabase.from('progress_photos').update({ adjustments: result.after }).eq('id', after.id); toast.success(t('tab.photosAligned')) } catch { setAlignError(t('tab.analysisError')) } finally { setIsAligning(false) } }
