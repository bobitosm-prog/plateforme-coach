...
import {
  ...
  SAVED_MEAL_SELECTION_ERROR_MESSAGE,
...
export default function NutritionTab({ coachMealPlan, todayKey, setModal, profile, supabase, userId, fetchAll }: NutritionTabProps) {
  ...
  const [savedMealSelection, setSavedMealSelection] = useState(
    createEmptySavedMealSelection<ReusableMeal>(),
  )
  ...
  useEffect(() => {
    ...
    savedMealSelectionRequest.current += 1
    savedMealReuseInFlight.current = false
    setSavedMealReusing(false)
    setSavedMealSelection(createEmptySavedMealSelection())
  ...
  function openSavedMealSelection(mealType: MealKey) {
    setUseSavedMealTarget(mealType)
    setSavedMealError(null)
    setShowSavedMeals(true)
    const request = ++savedMealSelectionRequest.current
    setSavedMealSelection(beginSavedMealSelection)
    void supabase.from('saved_meals').select(SAVED_MEAL_PROJECTION)
      .eq('user_id', userId).order('created_at', { ascending: false })
      .then((result: { data: readonly ReusableMeal[] | null; error: unknown | null }) => {
        setSavedMealSelection(previous => settleSavedMealSelection(
          previous,
          result,
          request === savedMealSelectionRequest.current,
        ))
      })
  }
