...
import {
  ...
  X, Clock, Plus, Flame, Activity, Moon, Trash2, UtensilsCrossed,
} from 'lucide-react'
              ...
              <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', zIndex: 60, display: 'flex', alignItems: 'flex-end' }}>
                <div style={{ background: '#1A1A1A', borderRadius: '24px 24px 0 0', padding: '24px 20px 40px', width: '100%', maxWidth: 480, margin: '0 auto' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <input value={newFood.name} onChange={e => setNewFood(p => ({ ...p, name: e.target.value }))} placeholder="Nom de l'aliment *"
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                      {([
                        ...
                        <div key={k}>
                          ...
                          <input type="number" inputMode="decimal" value={newFood[k]} onChange={e => setNewFood(p => ({ ...p, [k]: e.target.value }))} placeholder="0"
                    ...
                    {/* Cooked toggle */}
                    <button onClick={() => setNewFood(p => ({ ...p, is_cooked: !p.is_cooked }))} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderRadius: 10, background: newFood.is_cooked ? 'rgba(201,168,76,0.1)' : '#0A0A0A', border: `1px solid ${newFood.is_cooked ? '#C9A84C40' : '#2A2A2A'}`, cursor: 'pointer' }}>
      ...
      <nav className="bottom-nav" aria-label="Navigation principale">
        {([
          ...
          <button
            ...
            onClick={() => setSection(tab.key as 'dashboard' | 'messages' | 'calendar' | 'aliments' | 'profil')}
