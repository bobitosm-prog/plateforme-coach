...
const PATH = '/fr/nutrition/perte-de-poids'
...
export async function generateMetadata({
  ...
  return {
    title: WEIGHT_LOSS_CONTENT.seoTitle,
    description: WEIGHT_LOSS_CONTENT.description,
    alternates: {
...
export default async function WeightLossPage({
  ...
  const jsonLd = {
    ...
    url: CANONICAL_URL,
    name: WEIGHT_LOSS_CONTENT.title,
    description: WEIGHT_LOSS_CONTENT.description,
    inLanguage: 'fr',
    isPartOf: {
      '@id': `${SITE_URL}/#website`,
      ...
      <main>
        <article style={{ maxWidth: 920, margin: '0 auto', padding: '64px 24px 88px' }}>
          <header style={{ maxWidth: 800, marginBottom: 48 }}>
            <p style={{ color: '#c9a84c', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase' }}>Nutrition · Objectif</p>
            ...
            <p style={{ color: 'rgba(255,255,255,0.76)', fontSize: 18, lineHeight: 1.75 }}>{WEIGHT_LOSS_CONTENT.introduction}</p>
          ...
          <nav aria-label="Sommaire" style={{ background: '#0d0c0b', border: '1px solid rgba(201,168,76,0.25)', padding: 24, marginBottom: 56 }}>
            ...
            <ol style={{ marginBottom: 0, paddingLeft: 22 }}>
              {WEIGHT_LOSS_CONTENT.sections.map(section => (
          ...
          <section aria-labelledby="related-content" style={{ borderTop: '1px solid rgba(255,255,255,0.12)', paddingTop: 40, marginBottom: 56 }}>
            <h2 id="related-content" style={{ color: '#c9a84c' }}>Poursuivre avec MoovX</h2>
            <ul style={{ display: 'grid', gap: 14, padding: 0, listStyle: 'none' }}>
              {WEIGHT_LOSS_CONTENT.links.map(link => (
          ...
          <aside style={{ border: '1px solid rgba(201,168,76,0.3)', background: 'rgba(201,168,76,0.08)', padding: 24, lineHeight: 1.7 }}>
            {WEIGHT_LOSS_CONTENT.disclaimer}
          </aside>
