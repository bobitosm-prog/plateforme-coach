...
const PATH = '/fr/nutrition/proteines-par-jour'
...
export async function generateMetadata({
  ...
  return {
    title: DAILY_PROTEIN_CONTENT.seoTitle,
    description: DAILY_PROTEIN_CONTENT.description,
    alternates: {
...
export default async function DailyProteinPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  ...
  const jsonLd = {
    ...
    url: CANONICAL_URL,
    name: DAILY_PROTEIN_CONTENT.title,
    description: DAILY_PROTEIN_CONTENT.description,
    inLanguage: 'fr',
    isPartOf: {
      '@id': `${SITE_URL}/#website`,
      ...
      <main>
        <article style={{ maxWidth: 920, margin: '0 auto', padding: '64px 24px 88px' }}>
          <header style={{ maxWidth: 800, marginBottom: 48 }}>
            <p style={{ color: '#c9a84c', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase' }}>Nutrition · Protéines</p>
            ...
            <p style={{ color: 'rgba(255,255,255,0.76)', fontSize: 18, lineHeight: 1.75 }}>{DAILY_PROTEIN_CONTENT.introduction}</p>
          ...
          <nav aria-label="Sommaire" style={{ background: '#0d0c0b', border: '1px solid rgba(201,168,76,0.25)', padding: 24, marginBottom: 56 }}>
            ...
            <ol style={{ marginBottom: 0, paddingLeft: 22 }}>
              {DAILY_PROTEIN_CONTENT.sections.map(section => (
          ...
          <section aria-labelledby="related-content" style={{ borderTop: '1px solid rgba(255,255,255,0.12)', paddingTop: 40, marginBottom: 56 }}>
            ...
            <ul style={{ display: 'grid', gap: 14, padding: 0, listStyle: 'none' }}>
              {DAILY_PROTEIN_CONTENT.links.map(link => (
          ...
          <section aria-labelledby="sources" style={{ marginBottom: 48 }}>
            ...
            <ul style={{ lineHeight: 1.7 }}>
              {DAILY_PROTEIN_CONTENT.sources.map(source => (
          ...
          <aside style={{ border: '1px solid rgba(201,168,76,0.3)', background: 'rgba(201,168,76,0.08)', padding: 24, lineHeight: 1.7 }}>
            {DAILY_PROTEIN_CONTENT.disclaimer}
          </aside>
