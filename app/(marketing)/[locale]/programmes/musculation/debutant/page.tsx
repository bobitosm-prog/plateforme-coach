...
const PATH = '/fr/programmes/musculation/debutant'
...
export async function generateMetadata({
  ...
  return {
    title: BEGINNER_PROGRAM_CONTENT.seoTitle,
    description: BEGINNER_PROGRAM_CONTENT.description,
    alternates: {
...
export default async function BeginnerProgramPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  ...
  const jsonLd = {
    ...
    url: CANONICAL_URL,
    name: BEGINNER_PROGRAM_CONTENT.title,
    description: BEGINNER_PROGRAM_CONTENT.description,
    inLanguage: 'fr',
    isPartOf: {
      '@id': `${SITE_URL}/#website`,
      ...
      <main>
        <article style={{ maxWidth: 980, margin: '0 auto', padding: '64px 24px 88px' }}>
          <header style={{ maxWidth: 820, marginBottom: 48 }}>
            <p style={{ color: '#c9a84c', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase' }}>Musculation · Débutant</p>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.5rem, 8vw, 5rem)', lineHeight: 1, margin: '16px 0 24px' }}>
              {BEGINNER_PROGRAM_CONTENT.title}
            </h1>
            <p style={{ color: 'rgba(255,255,255,0.76)', fontSize: 18, lineHeight: 1.75 }}>{BEGINNER_PROGRAM_CONTENT.introduction}</p>
          ...
          <nav aria-label="Sommaire" style={{ background: '#0d0c0b', border: '1px solid rgba(201,168,76,0.25)', padding: 24, marginBottom: 56 }}>
            ...
            <ol style={{ marginBottom: 0, paddingLeft: 22 }}>
              {BEGINNER_PROGRAM_CONTENT.sections.map(section => (
          ...
          <aside style={{ border: '1px solid rgba(201,168,76,0.3)', background: 'rgba(201,168,76,0.08)', padding: 24, lineHeight: 1.7 }}>
            {BEGINNER_PROGRAM_CONTENT.disclaimer}
          </aside>
