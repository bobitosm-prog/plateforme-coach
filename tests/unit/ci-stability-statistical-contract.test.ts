...
describe('Phase 9 CI stability statistical contract', () => {
  ...
  it('documents append-only collection and does not claim current stability', () => {
    ...
    expect(contract).toContain('## Contrat historique V1 — conservé')
  ...
  it('formalizes the additive V2 sampling decision without weakening V1 gates', () => {
    const contract = readFileSync('docs/CI_STABILITY_STATISTICAL_CONTRACT.md', 'utf8')
    const normalizedContract = contract.replace(/\s+/g, ' ')
    expect(normalizedContract).toContain('Le modèle `phase-9-ci-stability-v2` est adopté le **2026-09-04**')
    expect(normalizedContract).toContain('V2 est additif et non rétroactif')
    expect(normalizedContract).toContain('nouveau GitHub `run_id` et `run_attempt=1`')
    expect(normalizedContract).toContain("Elle n'impose pas un SHA différent")
    expect(normalizedContract).toContain('nouvelle fenêtre V2 repart de zéro sur le nouveau SHA')
    expect(normalizedContract).toContain('une observation par heure')
    expect(normalizedContract).toContain('`schedule/cron`')
    expect(normalizedContract).toContain('`cancel-in-progress: false`')
    expect(normalizedContract).toContain('ne peuvent pas être le mécanisme principal des 150 observations')
    expect(normalizedContract).toContain('exactement sur les **150 dernières observations primaires admissibles de la fenêtre V2**')
    expect(normalizedContract).toContain('La fenêtre V2 peut démarrer uniquement après validation simultanée')
    expect(normalizedContract).toContain('168 tentatives théoriques')
    expect(normalizedContract).toContain('89,3 % de completion')
    expect(normalizedContract).toContain('marge de 18 tentatives')
    expect(normalizedContract).toContain('p95 nearest-rank strictement inférieur à 20 minutes')
    expect(normalizedContract).toContain('flaky rate strictement inférieur à 2 %')
    expect(normalizedContract).toContain('`unresolved = 0` et `unknown = 0`')
  })
})
