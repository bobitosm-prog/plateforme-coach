...
describe('Training V2 final polish contracts', () => {
  ...
  it('contains the active set editor at narrow mobile widths without shrinking touch targets', () => {
    ...
    expect(styles).toMatch(/\.stepper > button,[\s\S]*?\.rirOptions button\s*\{[^}]*min-width:\s*48px;[^}]*min-height:\s*48px;/)
    ...
    expect(narrowMobile).not.toMatch(/\.setEditorControls\s*\{[^}]*grid-template-columns:\s*1fr;/)
