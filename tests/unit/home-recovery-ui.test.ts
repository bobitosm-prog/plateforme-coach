...
describe('Home recovery interface', () => {
  ...
  it('keeps mask selections even when only the initially selected chest has an estimate', () => {
  ...
  it('renders the v2 neutral bodies and all pixel-aligned mask assets', () => {
    ...
    expect(modal).toContain('loadRecoveryMaskReaders')
    expect(modal).toContain('image.decode()')
    expect(modal).toContain('getImageData(0, 0, RECOVERY_MASK_WIDTH, RECOVERY_MASK_HEIGHT).data')
    expect(modal).toContain('rgba[index * 4 + 3]')
    expect(modal).toContain('onPointerUp={handlePointerUp}')
    expect(modal).toContain('resolveRecoveryPointerZoneFromMasks(')
    expect(modal).not.toContain('ZONE_SHAPES')
    expect(modal).not.toContain('HIT_AREA_SHAPES')
    expect(modal).not.toContain('<svg')
    expect(modal).not.toMatch(/<path\b|\spaths?\s*:/)
    ...
    expect(modal).toContain("type RecoveryMaskHitState = 'loading' | 'ready' | 'error'")
    expect(modal).toContain('data-mask-hit-state={maskHitState}')
    expect(modal).toContain("image.addEventListener('load', createReader")
    expect(modal).toContain("image.addEventListener('error', fail")
    expect(modal).not.toContain('RECOVERY_ATLAS_ASSETS')
    expect(modal).not.toContain('recoveryZoneFromAtlasPixel')
