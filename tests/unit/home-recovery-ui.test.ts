...
describe('Home recovery interface', () => {
  ...
  it('renders the v2 neutral bodies and all pixel-aligned mask assets', () => {
    ...
    expect(modal).toContain('onClick={handleClick}')
    expect(modal).toContain('resolveRecoveryClickZone(')
    expect(modal).toContain('data-hit-map="static"')
    ...
    expect(modal).not.toContain('loadRecoveryMaskReaders')
    expect(modal).not.toContain('new window.Image')
    expect(modal).not.toContain('drawImage')
    expect(modal).not.toContain('getImageData')
    expect(modal).not.toContain('onPointerUp')
    expect(modal).not.toContain('data-mask-hit-state')
