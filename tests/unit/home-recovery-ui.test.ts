...
describe('Home recovery interface', () => {
  ...
  it('renders the v2 neutral bodies and all pixel-aligned mask assets', () => {
    ...
    expect(modal).toContain('RECOVERY_ATLAS_ASSETS[view]')
    expect(modal).toContain('atlas.decode()')
    expect(modal).toContain('getImageData(x, y, 1, 1).data')
    expect(modal).toContain('onPointerUp={handlePointerUp}')
    expect(modal).toContain('resolveRecoveryPointerZone(')
