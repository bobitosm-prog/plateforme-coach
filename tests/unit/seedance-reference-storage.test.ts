...
describe('Seedance reference staging storage', () => {
  ...
  it.each([
    'https://example.com/storage/v1/object/sign/seedance-references/path?token=synthetic',
  ...
  it('classifies the Production host but refuses it for this provider tranche', () => {
    const value = 'https://njlzossopgknanhkzcbk.supabase.co/storage/v1/object/sign/seedance-references/path?token=synthetic'
    expect(classifySeedanceReferenceUrl(value)).toBe('production_https')
    expect(() => assertSeedanceProviderReferenceUrl(value)).toThrow()
  })
