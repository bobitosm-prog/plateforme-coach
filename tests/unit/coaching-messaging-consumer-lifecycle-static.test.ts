...
const coach = readFileSync('app/coach/hooks/useCoachDashboard.ts', 'utf8')
...
describe('messaging consumer lifecycle guards', () => {
  ...
  it('deduplicates realtime unread increments by persisted message ID', () => {
    ...
    expect(coach).toContain('globalRealtimeMessageIdsRef.current.has(m.id)')
  })
