...
> État observé sur les 137 migrations locales au 14 juillet 2026. Les matrices utilisent uniquement Supabase local. Les migrations `20260714211500_harden_payments_rls.sql` et `20260714224500_harden_coach_clients_writes.sql` corrigent les deux accès critiques révélés par la matrice.
...
| Coach lié actif | É lecture profil client absente; UPDATE prévue mais trigger cassé | A lecture relation; R mutations directes | A CRUD limité à création/consultation/révocation propre | N/A | A lecture des paiements du client lié; R toutes mutations |
...
- 3 scénarios d'écarts connus restent visibles sous forme de `RLS_KNOWN_GAP`.
...
Ces trois scénarios ne sont pas transformés en attentes normales. Ils émettent un avertissement à chaque exécution. Les anciens avertissements `payments` et `coach_clients` sont désormais des attentes bloquantes de refus.
