...
**À surveiller.** Statuts HTTP de `/api/stripe/connect`, raisons `AUTH_REQUIRED`, `IDENTITY_MISMATCH` ou `STRIPE_IDENTITY_INVALID`, `PROFILE_UNAVAILABLE` ou `RESOURCE_NOT_FOUND`, `ROLE_FORBIDDEN`, et état de `profiles.stripe_account_id` / `profiles.stripe_onboarding_complete`.

**Fenêtre dual-read Stripe Connect.** Les requêtes de logs, alertes, tests de contrat et procédures opérateur acceptent temporairement les paires exactes `IDENTITY_MISMATCH` / `STRIPE_IDENTITY_INVALID` et `PROFILE_UNAVAILABLE` / `RESOURCE_NOT_FOUND`. Le producteur Connect émet désormais les reasons canoniques. Les deux refus restent contractuellement `403` et leurs corps publics restent inchangés ; le statut générique `404` du descriptor `RESOURCE_NOT_FOUND` ne s'applique pas à cette route historique. La tolérance dual-read reste obligatoire pendant au moins une release complète observée après la bascule et jusqu'à la fermeture de la fenêtre de rollback.
...
**Fenêtre dual-read échec checkout.** Les requêtes de logs, alertes, tests de contrat et procédures opérateur acceptent temporairement exactement `CHECKOUT_FAILED` et `UPSTREAM_REJECTED`. Les producteurs plateforme et coach émettent désormais `UPSTREAM_REJECTED`. Les deux réponses restent contractuellement `500` avec leurs corps historiques inchangés ; le statut générique `502` du descriptor `UPSTREAM_REJECTED` ne s'applique pas à ces routes historiques. Cette tolérance reste obligatoire pendant au moins une release complète observée après la bascule et jusqu'à la fermeture de la fenêtre de rollback.
...
**Fenêtre dual-read persistance invitation.** Les consommateurs, tests de contrat et procédures opérateur acceptent temporairement exactement `INVITATION_CONSUMPTION_FAILED` et `PERSISTENCE_FAILED`. Les routes TypeScript validate/consume émettent désormais `PERSISTENCE_FAILED`; la RPC/migration historique reste inchangée et son code legacy est normalisé par consume. `/join` projette les deux codes vers le même état `temporary`, sans bascule vers `invalid`, sans changement de message public, de statut HTTP ou d'anti-énumération. Cette tolérance reste obligatoire pendant au moins une release complète observée après la bascule et jusqu'à la fermeture de la fenêtre de rollback.
