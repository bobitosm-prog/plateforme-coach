...
Un parcours critique est défini comme une entrée exécutable nommée unique du runner canonique, rattachée à une spécification métier principale. Le nombre de tests Playwright contenus dans cette spécification ne change pas le décompte et les suites de performance en sont exclues. La matrice cible comporte 15 parcours; six sont intégrés, dont le parcours inscription, authentification et reprise de session ajouté après qualification locale.
...
- Neuf parcours de la matrice cible restent planifiés; ce sous-batch n'intègre notamment pas encore Training, Nutrition, Progression, Realtime ou réconciliation Billing.
