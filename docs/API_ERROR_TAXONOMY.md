...
Pour la persistance lors de la consommation d'une invitation, le consommateur `/join` accepte temporairement exactement `INVITATION_CONSUMPTION_FAILED` et `PERSISTENCE_FAILED`. Les producteurs TypeScript validate/consume émettent désormais `PERSISTENCE_FAILED` avec leur statut et leur corps historiques. Les deux codes produisent le même état UX temporaire, jamais l'état invalide. La RPC SQL historique peut encore lever `INVITATION_CONSUMPTION_FAILED`; la route consume le normalise vers le code canonique sans modifier la migration. Le dual-read reste requis pendant au moins une release complète observée après la bascule et jusqu'à expiration de la fenêtre de rollback.
