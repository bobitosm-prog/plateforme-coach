...
> Audit mis à jour après la migration isolée de `adapt-workout`, le 21 juillet
> 2026. Aucun autre transport, prompt, modèle, quota, route ou consommateur
> n'est modifié par cette tranche.

## Verdict

**Phase 7 incomplète.** Les treize tâches de checklist sont cochées, mais la définition de terminé n'est pas satisfaite : 8 des 15 points d'entrée utilisent `AiProvider`; 7 restent sur transport historique. La Phase 8 ne doit pas être activée.
...
| Points d'entrée `AiProvider` | 8 | 8 fichiers route créent `createAnthropicProvider` |
| Points d'entrée historiques | 7 | 5 modules HTTP directs couvrant 6 features + 1 route SDK |
| Expressions HTTP Anthropic directes | 6 | dont 2 branches dans `analyze-progress-photo` |
| Clients SDK directs | 1 | `new Anthropic` dans les instructions d'exercice |
| Sites d'invocation runtime totaux | 8 | adaptateur partagé + 6 `fetch` directs + 1 SDK |
...
### Capacités des sept historiques
...
|---|---|---|---|
| Tous appels via provider | **unmet** | 8/15; 6 `fetch` directs + 1 SDK | migrer les 7 features historiques |
...
## Sept transports historiques et ordre recommandé

| Ordre | Feature(s) | Forme | Complexité / risque | Test manquant | Partage possible |
|---:|---|---|---|---|---|
| 1 | `analyze-meal-photo` | JSON multimodal | moyenne / moyen | route, taille et blocs image | analyses Nutrition/photos |
| 2 | `suggest-overload` | JSON + écriture | moyenne / élevé | échecs transport/insertion | outils Training |
| 3 | `generate-exercise-instructions` | SDK batch JSON | moyenne / élevé | batch partial et tokens | outils Training, contrat séparé |
| 4 | `analyze-body` | tool_use, 3 images | moyenne / élevé | URL, quota, outil invalide | analyses corps/photos |
| 5 | `analyze-progress-photo` | texte libre, 1–3 images | élevée / élevé | deux modes et texte absent | analyses corps/photos |
| 6 | `weekly-diagnostic` | tool + écritures/push | élevée / élevé | transport et échecs partiels | diagnostic manuel/cron |
| 7 | `weekly-diagnostic-cron` | même tool + batch | élevée / élevé | concurrence et agrégat | même service diagnostic |
...
`adapt-workout` conserve son golden exact, `max_tokens=800`, le même Sonnet,
les statuts d'authentification/limitation/usage et le corps de succès. Son signal
HTTP est désormais propagé, les erreurs fournisseur sont expurgées et la
finalisation distingue l'annulation sans retry, fallback, écriture ou nouveau
timeout réseau.

Prochaine tâche unique : **migrer `analyze-meal-photo` vers `AiProvider` avec tests de contrat, sans changement de prompt ni de réponse publique**.
