...
> Audit en lecture du code au HEAD `4840169`, le 21 juillet 2026. Aucun transport, prompt, modèle, quota, route ou consommateur n'est modifié par cette tranche.

## Verdict

**Phase 7 incomplète.** Les treize tâches de checklist sont cochées, mais la définition de terminé n'est pas satisfaite : seuls 7 des 15 points d'entrée utilisent `AiProvider`; 8 restent sur transport historique. La Phase 8 ne doit pas être activée.
...
| Points d'entrée `AiProvider` | 7 | 7 fichiers route créent `createAnthropicProvider` |
| Points d'entrée historiques | 8 | 6 modules HTTP directs couvrant 7 features + 1 route SDK |
| Expressions HTTP Anthropic directes | 7 | dont 2 branches dans `analyze-progress-photo` |
| Clients SDK directs | 1 | `new Anthropic` dans les instructions d'exercice |
| Sites d'invocation runtime totaux | 9 | adaptateur partagé + 7 `fetch` directs + 1 SDK |
| Littéraux fournisseur historiques hors builders | 9 | 8 usages/modèles demandés et 1 `ai_model` persisté |
...
| `analyze-meal-photo` | `POST /api/analyze-meal-photo` | HTTP direct | Sonnet direct | repas photo | JSON + Zod | quota, user |
| `suggest-exercise` | `POST /api/suggest-exercise` | `AiProvider` | registre Haiku | alternative | JSON + Zod | quota, user |
| `adapt-workout` | `POST /api/adapt-workout` | HTTP direct | Sonnet direct | adaptation | JSON + Zod | usage, user |
...
| `suggest-overload` | `POST /api/suggest-overload` | HTTP direct | Haiku direct | surcharge | JSON + Zod + écriture | usage, user |
...
### Capacités des huit historiques

| Feature | Annulation / timeout / retry | Tokens et coût | Erreurs et logs | Golden / fallback / test de contrat |
|---|---|---|---|---|
| `analyze-meal-photo` | aucun signal, timeout ou retry | métadonnées lues; coût via usage, sinon indisponible | `e.message` loggé et renvoyé | oui / no-fallback / route manquante |
| `adapt-workout` | aucun signal, timeout ou retry | métadonnées lues; coût via usage | `e.message` renvoyé | oui / no-fallback / route manquante |
...
| `suggest-overload` | aucun signal, timeout ou retry | métadonnées lues; coût via usage | statut, erreurs SQL et message d'exception exposables | oui / no-fallback / transport-écriture manquant |
...
Les builders de prompt sont séparés pour les huit. Le JSON/tool_use est validé par les schémas et parseurs communs; seule la sortie texte libre de progression photo ne passe pas par `aiFreeTextSchema`. Les principals sont issus de la session pour les sept routes utilisateur, du contrôle admin pour les instructions et de `CRON_SECRET` avec principal serveur + sujet pour le cron.
...
|---|---|---|---|
| Tous appels via provider | **unmet** | 7/15; 7 `fetch` directs + 1 SDK | migrer les 8 features historiques |
...
## Huit transports historiques et ordre recommandé

| Ordre | Feature(s) | Forme | Complexité / risque | Test manquant | Partage possible |
|---:|---|---|---|---|---|
| 1 | `adapt-workout` | JSON texte | faible / moyen | contrat route succès/erreur | outils Training |
| 2 | `analyze-meal-photo` | JSON multimodal | moyenne / moyen | route, taille et blocs image | analyses Nutrition/photos |
| 3 | `suggest-overload` | JSON + écriture | moyenne / élevé | échecs transport/insertion | outils Training |
| 4 | `generate-exercise-instructions` | SDK batch JSON | moyenne / élevé | batch partial et tokens | outils Training, contrat séparé |
| 5 | `analyze-body` | tool_use, 3 images | moyenne / élevé | URL, quota, outil invalide | analyses corps/photos |
| 6 | `analyze-progress-photo` | texte libre, 1–3 images | élevée / élevé | deux modes et texte absent | analyses corps/photos |
| 7 | `weekly-diagnostic` | tool + écritures/push | élevée / élevé | transport et échecs partiels | diagnostic manuel/cron |
| 8 | `weekly-diagnostic-cron` | même tool + batch | élevée / élevé | concurrence et agrégat | même service diagnostic |
...
Prochaine tâche unique : **migrer `adapt-workout` vers `AiProvider` avec tests de contrat, sans changement de prompt ni de réponse publique**.
