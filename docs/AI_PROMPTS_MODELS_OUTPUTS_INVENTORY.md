...
`progressive_overload_suggestions` reste une dépendance de schéma legacy : la
table n'apparaît ni dans les types Supabase générés ni dans une migration de
création/policy versionnée; seules deux migrations de suppression de compte la
référencent. La route utilise donc le service role après authentification,
garde invité et validations, avec `user_id` exclusivement issu de la session.
La migration provider ne prétend pas corriger cette divergence ni ajouter une
transaction : le résultat fournisseur validé peut exister sans insertion si
la mutation échoue.
...
| Diagnostic hebdomadaire manuel | `POST /api/weekly-diagnostic`; `HomeTab` | Système et prompt assemblant entraînement, nutrition, poids, bien-être, objectifs et profil lus côté serveur; objectifs Nutrition résolus sans faux zéro par `resolveWeeklyDiagnosticNutritionGoals` | Outil forcé via `AiProvider`, validé par `weeklyDiagnosticOutputSchema`; écrit diagnostic puis prochaine date, push best effort | Session, limite IP 3/min, usage journalisé sans quota DB; signal HTTP, erreurs expurgées. Une panne profil arrête avant IA/écritures; le raisonnement fournisseur reste persisté |
| Diagnostic hebdomadaire cron | `POST /api/weekly-diagnostic/cron`; planificateur serveur | Même générateur, même résolution des objectifs nullable et même invocation que le flux manuel | Même outil/validation/écritures; lots parallèles de 5, succès conservés et échecs comptés | `CRON_SECRET`, service role, principal serveur + sujet; une opération par utilisateur tenté, signal serveur, aucun retry/timeout fournisseur |
...
distincts et bornés; la progression photo valide désormais sa sortie avant de
répondre.
...
| Fallback | Registre commun 15/15; quatre partials historiques. La progression photo applique désormais sa policy `no_fallback`; seul le repli explicite comparaison → analyse simple sur image antérieure indisponible est préservé |
