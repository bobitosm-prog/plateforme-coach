...
async function sync() {
  ...
  // 1. Charger TOUS les exercices de la DB une fois (eviter N requetes)
  const { data: dbExercises, error: fetchError } = await supabase
    .from("exercises_db")
    .select("id, name");
  if (fetchError) throw fetchError;

  // Index DB par slug pour matching insensible aux accents/casse/espaces
  ...
  const exerciseNames = [...new Set(files.map((f) => path.parse(f).name.trim()))];
  ...
  for (const exerciseName of exerciseNames) {
    ...
    // 2. Si pas de match en DB, on log et on skip (vraie erreur, pas silencieuse)
    if (!dbExo) {
    ...
    const imageUrl = await uploadFile(pngPath, `${slug}/${slug}.png`, "image/png");

    // 3. Update via id (pas via name) - jamais d'ambiguite
  ...
  console.log(`\n🎉 Sync terminée : ${synced} synced, ${skipped} skipped, ${unmatched.length} unmatched`);
  if (unmatched.length > 0) {
    console.log(`\n⚠️  Exercices sans match en DB :`);
    unmatched.forEach((n) => console.log(`   - ${n}`));
    console.log(`\nVerifie les noms en DB ou cree les exercices manquants.`);
  }
}
